...
import { getQueryParams } from '../unit/utils'
...
export async function searchMovie(qes) {
  ...
  const url = `${API_BASE}/api/v1.0/Chat/Index?qes=${encodeURIComponent(
    qes
  )}&timespan=${timespan}&urlEncry=${urlEncry}&platform=${PLATFORM}`
...
export async function sendDanmu(data) {
  ...
  const blackWords =
    config && config.blackWords ? config.blackWords.split(',') : []

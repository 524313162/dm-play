    ...
    this.filter('emit', (danmu) => {
      ...
      switch (danmu.mode) {
        case 0: {
          const translateX =
            clientWidth -
            (this.getLeft(danmu.$ref) - this.getLeft(this.$player))
          danmu.$ref.style.transform = `translateX(${-translateX}px)`
          danmu.$ref.style.transition = 'transform 0s linear 0s'
          break
        }
    ...
    this.timer = window.requestAnimationFrame(async () => {
      if (this.art.playing && !this.isHide) {
        ...
        for (let index = 0; index < readys.length; index++) {
          ...
          danmu.$ref.style.border = danmu.border
            ? `1px solid ${danmu.color}`
            : null
          danmu.$ref.style.backgroundColor = danmu.border
            ? 'rgb(0 0 0 / 50%)'
            : null
          danmu.$ref.style.marginLeft = '0px'
            ...
            if (!this.isStop && top !== undefined) {
              ...
              switch (danmu.mode) {
                ...
                case 1:
                  ...
                  danmu.$ref.style.top = `${top}px`
                  danmu.$ref.style.marginLeft = `-${
                    danmu.$ref.clientWidth / 2
                  }px`
                  break
                default:
                  break
